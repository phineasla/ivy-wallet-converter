...
**2026-08-20 — strict UTF-8 validation removed.** User call: the fatal decode was too brittle — a few stray bytes in a real export blocked the entire conversion. `convertWith` now decodes leniently (`TextDecoder` without `fatal: true`), so invalid bytes become U+FFFD in the output instead of an `ok: false` error. BOM stripping is unchanged (still the decoder default). Spec story 17 and the Encoding decision updated to match; the seam test now pins the replacement-character behaviour. The `ok: false` path remains for unknown converter and file-read failures.
